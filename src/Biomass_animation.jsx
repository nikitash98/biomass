...
function Anim(props) {
  ...
  let stop_times = [0, 0.8, 1.7, 2.7, 4.0]
  const handleHover = (e) => {
    /*
      e.stopPropagation()
      setHovered(true)
  ...
  const handleUnhover = (e) => {
    /*
      setHovered(false)
      actions.ABC.paused = false;
      */
  }
  /*
  if (animations.length) {
    mixer = new THREE.AnimationMixer(nodes);
  ...
  if (actions) {
    ...
    if(actions.ABC) {
      actions.ABC.paused =false;
      if(stop_times[props.counter] < actions.ABC.time) {
        actions.ABC.setEffectiveTimeScale(-1)
  ...
  useFrame(()=> {
    console.log(actions.ABC.time)

    if(actions.ABC.getEffectiveTimeScale() < 0) {
      if (actions.ABC.time < stop_times[props.counter]) {
        actions.ABC.paused = true;
      }
    } else {
      if (actions.ABC.time > stop_times[props.counter]) {
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      <group name="Scene" >
        ...
        <mesh name="ABC001" geometry={nodes.ABC001.geometry} material={hovered ? materials['Material.019'] : materials['Material.022']} position={[4.92, 1.77, -25.39]} rotation={[Math.PI / 2, -Math.PI / 2, 0]} scale={0.63} onPointerOver = {handleHover} onPointerOut = {handleUnhover}>
